...
const AddExistContentsImpl: React.FC<Props> = ({
  ...
  const { data: relatedModel } = getModel(relation.one_model_id.toString());
  const { data: fields } = getFields(relation.one_model_id.toString());
  const { data: content } = getContents(relation.one_model_id.toString());
