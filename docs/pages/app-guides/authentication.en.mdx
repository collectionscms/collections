...
**Temporary Token (JWT)** are returned by the login endpoint. These tokens have a relatively short expiration date and are the safest to use.
The token is returned with the `refresh_token`. This `refresh_token` can be used by the refresh endpoint to obtain a new access token.
...
API keys are stored in plain text in `collections_users.api_key`.
