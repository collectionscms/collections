...
(async () => {
  ...
  const corrections = [
    [/(action.*)string \| number/gi, '$1keyof Superfast.IActions'],
    [/((addFilter|applyFilters).*)string \| number/gi, '$1keyof Superfast.IFilters'],
    [/((addSync|applySync).*)string \| number/gi, '$1keyof Superfast.ISyncFilters'],
  ];
